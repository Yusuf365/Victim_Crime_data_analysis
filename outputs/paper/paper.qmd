...
The data used in this paper was collected by the OpenDataToronto Library [@rOpenDataToronto]. The specific dataset used in this research is the
...
The Table 1 showcases the cleaned dataset from the raw data extracted from the 'Police-Annual-Statistical-Report-Victims-of-Crimes' [@citeDataset].
...
# Assuming cleaned_dataset is your data frame and it has the columns 'report_year', 'age_group', and 'victim_count'
...
crimes_age_subtype <- cleaned_dataset %>%
  ...
  summarise(total_count = sum(victim_count)) # Ensure grouped data is ungrouped after summarization
